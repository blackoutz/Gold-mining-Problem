...
def running(row, col):
    ...
    if 'G' in map[row][col] and not 'GM' in map[row][col]:
        ...
        # cost = 0
        # newMap = map[:]
       # shortestPath = findShortestPath(newMap,0,0,mine,lastPath,cost)
        allMine.append(mine)
...
def findShortestPath(newMap,row,col,mine,lastPath,cost):
...
cost = 0
newMap = map[:]
shortestPath = findShortestPath(newMap,0,0,allMine[0],lastPath,cost)
