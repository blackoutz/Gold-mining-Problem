...
def running(row, col, depth, depthLimit, copyMap):
    ...
    if 'Mark' in copyMap[row][col]:
        return None
    copyMap[row][col]['Mark'] = 1

    if 'Oil' in copyMap[row][col] and not 'OilMark' in copyMap[row][col]:
        OilWell = []
        ...
        for OilYield in OilWell:
            newMap = deepcopy(copyMap)
            x, y = OilYield['Pos']
            costInYield += newMap[x][y]['Cost']
            path, cost = findShortestPath(0, 0, newMap, x, y)
    ...
    running(row+1, col, depth+1, depthLimit, copyMap)
    running(row-1, col, depth+1, depthLimit, copyMap)
    running(row, col+1, depth+1, depthLimit, copyMap)
    running(row, col-1, depth+1, depthLimit, copyMap)
...
def findOil(row, col, OilWell):
    ...
    if not ('Oil' in map[row][col]):
        ...
        OilWell.append({'Value': map[row][col]['Oil'], 'Pos': [row, col]})
...
def findShortestPath(row,col,newMap,x,y):
...
def ucs(node, goal, map):
    ...
    while queue:
        ...
        for child in children(point, map):
            # Calculate the basic cost
            x, y = child
            child_cost = map[x][y]['Cost']
